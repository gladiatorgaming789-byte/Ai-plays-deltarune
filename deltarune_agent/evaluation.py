...
def calculate_metrics(events: Iterable[dict[str, object]]) -> RunMetrics:
    rows = list(events)
    rooms: set[str] = set()
    ...
    for event in rows:
        ...
        if state == "menu":
            menu_steps += 1
        if "try interaction" in reason or "interact" in reason and action == "confirm":
            interactions += 1
        if "story progress" in reason or "scripted sequence" in reason:
            story_progress += 1
        ...
        if isinstance(telemetry, dict):
            telemetry_steps += 1
            room = str(telemetry.get("room_name") or telemetry.get("room_id") or "unknown")
            rooms.add(room)
            x = telemetry.get("player_x", telemetry.get("x"))
            y = telemetry.get("player_y", telemetry.get("y"))
            try:
                cells.add((room, int(float(x) // 8), int(float(y) // 8)))
    ...
    steps = len(rows)
    coverage = telemetry_steps / steps if steps else 0.0
...
def compare_metrics(baseline: RunMetrics, candidate: RunMetrics) -> dict[str, float]:
    ...
    for key, baseline_value in baseline.as_dict().items():
        ...
        if isinstance(baseline_value, (int, float)) and isinstance(candidate_value, (int, float)):
            result[key] = round(float(candidate_value) - float(baseline_value), 4)
    return result


def write_metrics(run_directory: Path, metrics: RunMetrics) -> Path:

...
def _metadata(
    *,
    ...
    result: dict[str, object] = {
        ...
        "exporter": {
            "tool": "AI Plays Deltarune direct-CSX DeltaMod Builder"
        },
    }
...
def _modding_xml(chapters: tuple[int, ...], payload_label: str) -> str:
    return "".join(
        f'<patch type="xdelta" patch="./{_payload_name(chapter, payload_label)}" '
    ...
    *,
    ...
    with zipfile.ZipFile(package) as archive:
        ...
        for line in lines:
            ...
            if match is None:
                raise ValueError(
                    "modding.xml must use xdelta-type CSX patches targeting exact "
        ...
        if len(payload_hashes) != 1:
            raise ValueError(
                "all per-chapter CSX payloads must contain identical source"
            )
        expected = (
            set(expected_chapters)
            if expected_chapters is not None
            else declared_chapters
        )
        ...
        if needed_files is not None:
            ...
            if needed_chapters != declared_chapters:
                raise ValueError(
                    "neededFiles and modding.xml target different chapters"
                )
